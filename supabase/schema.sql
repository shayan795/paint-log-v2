...
-- ----------------------------------------------------------------------------
-- 使い方(新しいDBを作るときの正しい順番):
--   1) このファイル(schema.sql)を SQL Editor で実行
--   2) supabase/seed_paints_realigned.sql を実行  ★こちらが正本
--      （seed_paints.sql は並び順がフロントの PAINTS 配列とズレた旧ファイル。使わないこと）
--   3) migrations/001 → 002 → 003 → 004 → 005 → 006 → 007 を連番順に実行  ★必須
--      このファイルは「初期設計」であり、その後のセキュリティ修正は migrations 側にある。
--      2)3) を飛ばすと、修正前の脆弱な定義のままになる。
--   4) Storageバケット recipes / profile と owner=auth.uid() のポリシーを作成
...
-- ============================================================================
-- このファイルはここまで。★まだ完了ではない★
--
-- 次に必ず実行すること:
--   1) supabase/seed_paints_realigned.sql   … 塗料512色（★正本。seed_paints.sql は使わない）
--   2) migrations/001 → 007 を連番順に      … その後のセキュリティ修正一式
--
-- migrations に入っている主な修正（このファイル単体には含まれていない）:
--   001 events保護 / 002 検索RPC / 003 コメントNG語
--   004 通報・問い合わせのレート制限をSECURITY DEFINER化 ほか
--   005 view_count初期値・profiles自己昇格・events偽装・grid上限・コメント整合・退会時の画像削除
--   006 コメント系イベント名の許可
--   007 created_at偽装の遮断 / 通知爆撃の抑制 / 通知の削除許可 / 閲覧数の上限 /
--       popular_methodsの集計修正 / paints索引 / drafts・events容量上限 / user_id形式 /
--       profilesの内部列を匿名から遮断
-- ============================================================================
