...
__name(getRecipeFromSupabase, "getRecipeFromSupabase");
function buildDescription(rec, authorLabel) {
  const grid = rec && rec.grid || {};
  const rows = Array.isArray(grid.rows) ? grid.rows : [];
  const paintNames = /* @__PURE__ */ new Set();
...
async function serveRecipePage(env, id) {
  ...
  if (rec) {
    ...
    const authorLabel = prof.display_name && prof.display_name.trim() ? prof.display_name.trim() : prof.user_id ? "@" + prof.user_id : (rec.author_label || "").trim();
    ...
    const block = `<meta property="og:type" content="article"><meta property="og:site_name" content="\u5857\u88C5\u30EC\u30B7\u30D4\u9332 / Paint Log"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(desc)}"><meta property="og:url" content="${esc(pageUrl)}"><meta property="og:image" content="${esc(image)}"><meta property="og:image:width" content="1080"><meta property="og:image:height" content="1080"><meta property="og:locale" content="ja_JP"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${esc(title)}"><meta name="twitter:description" content="${esc(desc)}"><meta name="twitter:image" content="${esc(image)}">`;
...
__name(serveRecipePage, "serveRecipePage");
var worker_default = {
